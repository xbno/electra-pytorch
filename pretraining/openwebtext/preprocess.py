...
def preprocess_owt(
    ...
    if n_dataset_building_processes == 1:
        feature_set_paths = preprocess_owt_job(
            tokenizer,
            src_dir,
            tmp_dir,
            trg_dir,
        ...
        preprocess_owt_job_partial = partial(
            preprocess_owt_job,
            tokenizer,
            src_dir,
            tmp_dir,
        ...
        feature_sets = pool.map(
            preprocess_owt_job_partial, range(n_dataset_building_processes)
        )
        feature_set_paths = [
            file_path for feature_set in feature_sets for file_path in feature_set
        ]

    return feature_set_paths


def preprocess_owt_job(
    tokenizer,
    src_dir,
    tmp_dir,
    trg_dir,
    ...
    for archive_id, archive in enumerate(job_archives[job_id]):
        ...
        logger.info(
            f"Job {job_id}: Processing {archive_id}/{len(job_archives[job_id])} {src_dir / archive}."
        )
...
def main():
    ...
    preprocess_owt(
        ...
        n_tensors_per_file=args.n_tensors_per_file,
